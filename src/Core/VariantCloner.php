 ...
 * The _splitevo_variant meta flag is used to hide them everywhere.
 */
class VariantCloner
{
    /**
     ...
     * Adds the _splitevo_variant flag so the post is hidden from
    ...
    public static function clone(int $post_id)
    {
        ...
        update_post_meta($clone_id, '_splitevo_variant', '1');
        // Explicit reference to the original so handle_apply() doesn't rely on post_parent.
        update_post_meta($clone_id, '_splitevo_control_post_id', $post_id);

        // Copy post meta, skipping WP internals and SplitEvo-managed keys.
